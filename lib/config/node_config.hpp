...
struct NodeConfig {
  ...
  void setDefaults(const char* defaultNodeId);
};
...
ConfigValidationResult validateConfig(const NodeConfig& config);
bool nodeIdValid(const char* nodeId);
bool radioConfigEqual(const NodeConfig& lhs, const NodeConfig& rhs);
